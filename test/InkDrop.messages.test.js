...
contract('InkDrop (message functions)', async accounts => {
  ...
  it('...unlike message extended', async () => {
    let inkdropInstance = await InkDrop.deployed()
    let msg = await inkdropInstance.getMessage(0, { from: accounts[9] })
    assert.equal(msg[4].length, 1, 'The message should have 1 like.')
    await inkdropInstance.likeMessage(0, { from: accounts[9] })
    await inkdropInstance.likeMessage(0, { from: accounts[7] })

    msg = await inkdropInstance.getMessage(0, { from: accounts[9] })
    assert.equal(msg[4].length, 3, 'The message should have 3 like.')
    assert.equal(msg[4][0], accounts[8], 'The user address should be stored.')
    assert.equal(msg[4][1], accounts[9], 'The user address should be stored.')
    assert.equal(msg[4][2], accounts[7], 'The user address should be stored.')

    await inkdropInstance.unlikeMessage(0, { from: accounts[9] })
    msg = await inkdropInstance.getMessage(0, { from: accounts[9] })
    assert.equal(msg[4].length, 2, 'The message should now have 2 likes.')
    assert.equal(msg[4][0], accounts[8], 'The user address should be stored.')
    assert.equal(msg[4][1], accounts[7], 'The user address should be stored.')

    await inkdropInstance.unlikeMessage(0, { from: accounts[8] })
    msg = await inkdropInstance.getMessage(0, { from: accounts[8] })
    assert.equal(msg[4].length, 1, 'The message should now have 1 likes.')
    assert.equal(msg[4][0], accounts[7], 'The user address should be stored.')
  })
})
