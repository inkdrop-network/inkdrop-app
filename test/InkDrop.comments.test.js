...
contract('InkDrop (comment functions)', async accounts => {
  ...
  it('...comment message extended', async () => {
    ...
    assert.equal(msg1[6].length, 3, 'The comment should now have 3 comments.')
    let comment1 = await inkdropInstance.getComment(msg1[6][1], { from: accounts[7] })
    assert.equal(comment1[2], accounts[8], 'The comment should be written by the defined address.')
    let comment2 = await inkdropInstance.getComment(msg1[6][2], { from: accounts[7] })
    assert.equal(comment2[2], accounts[9], 'The comment should be written by the defined address.')
    ...
    assert.equal(comment3[2], accounts[8], 'The comment should be written by the defined address.')
  })

  it('...get comment', async () => {
    let inkdropInstance = await InkDrop.deployed()
    let comment = await inkdropInstance.getComment(2, { from: accounts[7] })
    assert.equal(comment[0], 1, 'The message should the defined parent id.')
    assert.equal(comment[1], 'Hello comment world4', 'The message should have the defined content.')
    assert.equal(comment[2], accounts[8], 'The message should be written by the defined address.')
    assert.isTrue(new Date(comment[3] * 1000).getTime() > 0, 'Timestamp should be greater 0.')
    // assert.isTrue(new Date(comment[4] * 1000).getTime() > 0, 'Timetolive should be greater 0.')
    assert.equal(comment[5].toNumber(), 0, 'The comment should have 0 likes.')
    assert.equal(comment[6].toNumber(), 0, 'The comment should have 0 drops.')
    // assert.equal(comment[7].length, 0, 'The message should have 0 comments.')
  })

  it('...get comment with id out of bounce', async () => {
    let inkdropInstance = await InkDrop.deployed()
    try {
      await inkdropInstance.getComment(5, { from: accounts[7] })
      assert.fail('Should throw error.')
    } catch (error) {
      const revertFound = error.message.search('revert') >= 0
      assert.equal(revertFound, true, `Expected "revert", got ${error} instead`)
